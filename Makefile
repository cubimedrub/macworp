...
RELEASED_CONTAINER := ghcr.io/cubimedrub/macworp-dev-v2:latest
NIGHTLY_CONTAINER := cubimedrub/macworp-dev-v2:nightly
...
quickstart-up:
	...
	docker pull "${PUBLIC_REGISTRY}/macworp:${MACWORP_VERSION}"
	...
	env MACWORP_CONTAINER=${RELEASED_CONTAINER} DOCKER_SOCKET_PATH=${DOCKER_SOCKET_PATH} PROJECT_DIR_ABSOLUTE=${PROJECT_DIR_ABSOLUTE} SSL_DIR=${SSL_DIR} MACWORP_HOSTNAME=${MACWORP_HOSTNAME} MACWORP_FUSIONAUTH_PROTOCOL=https MACWORP_FUSIONAUTH_PORT=16161 \
...
quickstart-nightly-up:
	...
	env DOCKER_BUILDKIT=1 docker build -t "${NIGHTLY_CONTAINER}" -f docker/macworp.dockerfile .
	...
	# Start docker-compose in separate docker-compose project called macworp-quickstart, combining the two docker-compose files
	env MACWORP_CONTAINER=${NIGHTLY_CONTAINER} DOCKER_SOCKET_PATH=${DOCKER_SOCKET_PATH} PROJECT_DIR_ABSOLUTE=${PROJECT_DIR_ABSOLUTE} SSL_DIR=${SSL_DIR} MACWORP_HOSTNAME=${MACWORP_HOSTNAME} MACWORP_FUSIONAUTH_PROTOCOL=https MACWORP_FUSIONAUTH_PORT=16161 \
