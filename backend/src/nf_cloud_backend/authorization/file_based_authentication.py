...
# internal imports
from nf_cloud_backend import file_auth_databases, app
...
class FileBasedAuthentication(AbstractAuthentication):
    ...
    def login(cls, request: Request, provider: str) -> Response:
        ...
        if Configuration.values()["frontend_host_url"] is None:
            return redirect(f"{request.host_url}{cls.FRONTEND_FILE_BASED_LOGIN_FORM}?callback={callback_url}")
        else:
            return redirect(
    ...
    def callback(cls, request: Request, provider: str) -> Response:
        ...
        # Get auth DB
        auth_db: Optional[Dict[str, Dict[str, str]]] = file_auth_databases.get(provider, None)
        if auth_db is None:
            return jsonify({
                "errors": {
                    "general": "Provider not supported."
        ...
        try:
            ...
            app.logger.error(f"Error while authenticating user: {err}") # pylint: disable=no-member
            # Do not give any information which was wrong incase it was a brute force effort
            return jsonify({
                "errors": {
                    "general": "Login ID or password wrong."
                }
            }), 400
        ...
        token = JWT.create_auth_token(
            ...
            user,
            provider_client_config["expires_in"] + int(datetime.datetime.utcnow().timestamp()),
        )
