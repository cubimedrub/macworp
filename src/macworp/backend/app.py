...
from macworp.backend.controllers import healthcheck

TEMPORATY_DEV_MODE_CONFIG_PATH = Path(tempfile.gettempdir()).joinpath(
    f"{os.getuid()}.macworp.dev.config.yml"
)
...
def start_app(config: Configuration):
    ...
    if not config.development:
        ...
        descriptor = os.open(
            TEMPORATY_DEV_MODE_CONFIG_PATH,
            os.O_WRONLY  # access mode: write only
            | os.O_CREAT  # create if not exists
            | os.O_TRUNC,  # truncate the file to zero)
            0o600,  # permissions: user read/write only
        )
