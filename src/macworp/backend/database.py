...
def is_database_connectable(config: Configuration) -> bool:
    """Checks if the database is connectable.
    ...
    -------
    bool
        True if the database is connectable, false otherwise.

    Raises
    ------
    e
        Raises any SQLAlchemyError that is not an OperationalError.
    """
    try:
        with sqlalchemy_create_engine(
            config.backend.database, pool_size=1
        ).connect() as _conn:
            return True
