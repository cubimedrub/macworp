...
class LogEntry:
    ...
    def __init__(
        self, request: Request, status: int, json: Any, login: UserLogin | None
    ):
...
class Test(unittest.TestCase):
    ...
    def send(
        ...
        self.assert_(
            match_partial_json(actual_json, json),
            f"JSON doesn't match: Expected {json}, got {actual_json}",
        )
        self.assertEqual(
            actual_status,
            status,
            f"Status doesn't match: Expected {status}, got {actual_status}",
        )
        ...
        if login.login_id not in self.token_cache:
            self.token_cache[login.login_id] = self.send(
                Request(
                    ...
                    json={"login_id": login.login_id, "password": login.password},
                ),
                200,
                {"jwt": DONT_CARE},
            ).json()["jwt"]
    ...
    def as_user(
        self, login: UserLogin | None, request: Request, status: int, json: PartialJson
    ):
        """
