...
import re
from typing import Any, ClassVar, Dict, Iterator, List, Optional, Self, Tuple
...
class RabbitMQConfiguration(BaseModel):
    ...
    url: str = "amqp://admin:developer@127.0.0.1:5675/%2f"
...
class Configuration(BaseModel):
    ...
    def from_file(cls, path: Path) -> Self:
        ...
        plain_config = path.read_text(encoding="utf-8")
        Path("/tmp/macpepdb.debug.resolved.yml").write_text(
            cls.env_resolver(plain_config)
        )
        return parse_yaml_raw_as(cls, cls.env_resolver(plain_config))
