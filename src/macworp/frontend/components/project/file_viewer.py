...
class FileViewer:
    ...
    def _load_metadata(self, filepath: str) -> Dict[str, str]:
        ...
        metadata_path = os.path.join(
            "projects", str(self.project_id), "results", metadata_filename
        )
        try:
    ...
    def _create_image_viewer(self, filepath: str, title: str, description: str) -> None:
        ...
        with ui.expansion(title, icon="image").classes("w-full"):
            ...
            with ui.row().classes("w-full justify-center"):
                ui.image(
                    f"{self.config.projects_path}/{self.project_id}/{filepath}"
                ).classes("max-w-full max-h-96")
    ...
    def _create_plotly_viewer(
        self, filepath: str, title: str, description: str
    ) -> None:
        ...
        with ui.expansion(title, icon="analytics").classes("w-full"):
            ...
            try:
                full_path = os.path.join(
                    "projects", str(self.project_id), "results", filepath
                )
                ...
                ui.label(f"Fehler beim Laden der Plotly-Daten: {e}").classes(
                    "text-red-500"
                )
    ...
    def _create_text_viewer(self, filepath: str, title: str, description: str) -> None:
        ...
        with ui.expansion(title, icon="text_snippet").classes("w-full"):
            self._add_metadata_info(description)
            try:
                full_path = f"{self.config.projects_path}/{self.project_id}/{filepath}"
