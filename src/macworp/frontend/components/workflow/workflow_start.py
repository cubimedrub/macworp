...
from macworp.frontend.services.workflow_execution_service import (
    WorkflowExecutionService,
)
from macworp.frontend.services.workflow_parameter_service import (
    WorkflowParameterService,
)
...
class WorkflowStart:
    ...
    def __init__(self, project_id: int, config: Configuration, auth_token: str):
        ...
        self.workflows_service = WorkflowService(config, auth_token)
    ...
    async def workflow_selection(self) -> None:
        ...
        ) as dialog, ui.card().classes("w-full"):
    ...
    async def _render_parameter_inputs(
        self, selected_workflow: Dict, params: List[Dict]
    ) -> None:
        """
        ...
        with self.dialog_content:
            ...
            ui.button(
                "Delete Workflow",
                on_click=lambda w=selected_workflow: self.workflows_service.delete_workflow(
                    w
                ),
            )
    ...
    async def _render_single_parameter(self, param: Dict) -> None:
        ...
        if param_type == "file-glob":
            ...
            input_element = ui.select(
                options=options, label=param["label"], value=param.get("value")
            ).tooltip(param["desc"])
            self.parameter_inputs[param_id] = input_element
    ...
    async def _start_workflow_with_params(self) -> None:
        ...
        result = await self.execution_service.start_workflow_with_parameters(
            self.project_id, self.workflow, workflow_parameters
        )
