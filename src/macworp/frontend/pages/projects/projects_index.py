...
class ProjectsIndex:
    ...
    def __init__(self, config: Configuration, auth_token: str):
        ...
        self.new_project_dialog = ProjectEditTable(None, None, config, auth_token)
    ...
    def _render_project_card(self, project: Dict[str, Any]) -> None:
        ...
        with self.projects_list:
            with ui.card().classes("relative w-full mb-2"):
                ...
                ui.button(
                    "Share", on_click=lambda p=project: self._share_project(p)
        ...
        with (
            ...
            with ui.card().classes("w-full"):
                ...
                access_rights = ui.select(
                    label="Access rights", options=["read", "write"], value="read"
                )
            ...
            async def confirm_change() -> None:
                ...
                response = await self.project_service.add_share_project(
                    added_owner.value, project, project["id"], write_access
                )
    ...
    async def show(self) -> None:
        ...
        with ui.row().classes("w-full justify-between items-center"):
            ui.button("Start new project", on_click=self._new_project).classes(
                "btn btn-primary btn-sm"
            )

        self.projects_list = ui.column().classes("w-full")
        self.pagination = ui.pagination(
            value=self.current_projects_page, min=1, max=1, on_change=self._go_to_page
        ).classes("mt-4")
