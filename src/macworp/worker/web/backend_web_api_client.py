...
from macworp.utils.constants import (
...
class BackendWebApiClient:
    ...
    def __init__(
        ...
        """
        self.macworp_base_url = macworp_base_url
        self.credentials_hash = hash_worker_credentials(
            macworp_api_user, macworp_api_password
        )
        self.headers = self.HEADERS.copy()
        self.headers["Authorization"] = f"Basic {self.credentials_hash}"
    ...
    def get_workflow(self, workflow_id: int):
        ...
        """
        url = f"{self.macworp_base_url}/api/workflows/{workflow_id}"
    ...
    def is_project_ignored(self, project_id: int) -> bool:
        ...
        ValueError
            If the request was not successful.
        """
        url = f"{self.macworp_base_url}/api/projects/{project_id}/is-ignored"
        for i in range(self.__class__.API_CALL_TRIES):
            try:
                with requests.get(
                    url,
    ...
    def post_finish(self, project_id: int):
        ...
        ValueError
            If the request was not successful.
        """
        url = f"{self.macworp_base_url}/api/projects/{project_id}/finished"
        for i in range(self.__class__.API_CALL_TRIES):
            try:
                with requests.post(
                    url,
    ...
    def post_weblog(
        ...
        for i in range(self.__class__.API_CALL_TRIES):
            try:
                with requests.post(
                    f"{self.macworp_base_url}/api/projects/{project_id}/workflow-log",
                    headers=headers,
                    timeout=self.__class__.TIMEOUT,
                    verify=self.verify_cert,
                    data=log,
